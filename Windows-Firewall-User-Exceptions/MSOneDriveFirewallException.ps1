<#
    ...
    .DESCRIPTION
    Install:   C:\Windows\SysNative\WindowsPowershell\v1.0\PowerShell.exe -ExecutionPolicy Bypass -Command .\MSOneDriveFirewallException.ps1 -Force
...
Function Set-OneDriveFWRule($ProfileObj) {
# Setting up the inbound firewall rule required for optimal Microsoft OneDrive Co-Authoring within a LAN.
    
    Write-Verbose "Identified the current user as: $($ProfileObj.Name)" -Verbose
    $InstalledOneDriveVersion = (get-childitem -Path (Join-Path -Path $ProfileObj.FullName -ChildPath "AppData\Local\Microsoft\OneDrive") -Directory | where-object {$_.Name -match "^[0-9][0-9].[0-9][0-9][0-9][0-9]"}).Name
    $OneDriveCoAuthPath = "AppData\Local\Microsoft\OneDrive\" + $InstalledOneDriveVersion + "\Filecoauth.exe"
    $progPath = Join-Path -Path $ProfileObj.FullName -ChildPath $OneDriveCoAuthPath
    ...
    if ((Test-Path $progPath) -or ($Force)) {
        if ($Force) {          
            ...
            New-Item -Path "HKLM:\SOFTWARE\$AADNameNoBlanks\" -Name "MSOneDriveFirewallException"
            New-ItemProperty -Path "HKLM:\SOFTWARE\$AADNameNoBlanks\MSOneDriveFirewallException" -Name $ProfileObj.FullName -PropertyType "String" -Value $progPath -Force            
        }
        
        if (-not (Get-NetFirewallApplicationFilter -Program $progPath -ErrorAction SilentlyContinue)) {
            $ruleName = "Filecoauth.exe for user $($ProfileObj.Name)"
       ...
       $Message = "Filecoauth not found in $progPath - use the force parameter to override."
       Throw "$Message"
    }   
}
...
Try {   
    ...
    Set-OneDriveFWRule -ProfileObj (Get-LoggedInUserProfile)
